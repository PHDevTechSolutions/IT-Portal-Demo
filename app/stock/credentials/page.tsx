...
import { Loader2, Key, Save, RefreshCw, CheckCircle2, Eye, EyeOff, AlertTriangle, Database } from "lucide-react";
import {
...
const EMPTY = {
  ...
  collection_name: "suppliers", label: "Default",
};

function Field({ label, value, onChange, placeholder, secret = false, hint }: {
      ...
      <div className="relative">
        ...
        {secret && (
          <button type="button" onClick={() => setShow(s => !s)} className="absolute right-2 top-1/2 -translate-y-1/2">
            {show ? <EyeOff className="size-3" style={{ color: C.dim }} /> : <Eye className="size-3" style={{ color: C.dim }} />}
          </button>
        )}
      </div>
    </div>
...
export default function StockCredentialsPage() {
  ...
  const fetchCreds = async () => {
    setLoading(true);
    try {
      const res  = await fetch("/api/stock/credentials", { cache: "no-store" });
      ...
      if (json.success && json.credentials) {
        ...
        setForm({
          ...
          app_id:              c.app_id              ?? "",
          collection_name:     c.collection_name     ?? "suppliers",
  ...
  const handleSave = async () => {
    if (!form.api_key.trim() || !form.auth_domain.trim() || !form.project_id.trim()) {
      toast.error("API Key, Auth Domain, and Project ID are required"); return;
    }
    setSaving(true);
    try {
      const res  = await fetch("/api/stock/credentials", {
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      await fetch("/api/stock/credentials", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify(form),
      });
      ...
      if (json.success) {
        setTestResult("ok");
        toast.success(`Connection OK — found ${json.total} supplier(s) in "${form.collection_name}"`);
          ...
          <header className="relative z-10 flex h-11 shrink-0 items-center gap-2 px-4 border-b bg-[#080d12]" style={{ borderColor: C.border }}>
            ...
            <Breadcrumb>
              <BreadcrumbList>
                ...
                <BreadcrumbItem>
                  <BreadcrumbPage className="text-[10px] uppercase tracking-widest font-bold" style={{ color: C.accent }}>Firebase Credentials</BreadcrumbPage>
                </BreadcrumbItem>
              </BreadcrumbList>
            </Breadcrumb>
          ...
          <div className="relative z-10 shrink-0 flex items-center gap-3 px-4 py-3 border-b" style={{ borderColor: C.border, backgroundColor: C.panel }}>
            ...
            <div>
              ...
              <p className="text-[10px] uppercase tracking-wider mt-0.5" style={{ color: C.muted }}>
                Used by Stock · Suppliers · Saved to Supabase
              </p>
            </div>
            ...
            {loading ? (
              ...
              <div className="max-w-xl mx-auto space-y-6">
                <div className="flex items-start gap-3 px-4 py-3 border" style={{ borderColor: "#fbbf2440", backgroundColor: "rgba(251,191,36,0.05)" }}>
                  ...
                  <div>
                    <p className="text-[10px] font-bold" style={{ color: "#fbbf24" }}>Used only by the Stock module</p>
                    <p className="text-[9px] mt-0.5" style={{ color: C.dim }}>Suppliers page reads from the Firebase project configured here.</p>
                  </div>
                </div>

                <div className="space-y-4">
                  <h2 className="text-[9px] font-bold uppercase tracking-widest pb-1 border-b" style={{ color: C.accent + "80", borderColor: C.border }}>Label</h2>
                  <Field label="Label" value={form.label} onChange={v => setField("label", v)} placeholder="e.g. Production" hint="For reference only" />
                </div>

                <div className="space-y-4">
                  <h2 className="text-[9px] font-bold uppercase tracking-widest pb-1 border-b" style={{ color: C.accent + "80", borderColor: C.border }}>Firebase Project Config</h2>
                  <Field label="API Key *" value={form.api_key} onChange={v => setField("api_key", v)} placeholder="AIzaSy..." secret hint="Firebase Console → Project Settings" />
                  <div className="grid grid-cols-2 gap-3">
                    <Field label="Auth Domain *" value={form.auth_domain} onChange={v => setField("auth_domain", v)} placeholder="project.firebaseapp.com" />
                    <Field label="Project ID *" value={form.project_id} onChange={v => setField("project_id", v)} placeholder="your-project-id" />
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <Field label="Storage Bucket" value={form.storage_bucket} onChange={v => setField("storage_bucket", v)} placeholder="project.appspot.com" />
                    <Field label="Messaging Sender ID" value={form.messaging_sender_id} onChange={v => setField("messaging_sender_id", v)} placeholder="123456789" />
                  </div>
                  <Field label="App ID" value={form.app_id} onChange={v => setField("app_id", v)} placeholder="1:xxx:web:xxx" />
                </div>

                <div className="space-y-4">
                  <h2 className="text-[9px] font-bold uppercase tracking-widest pb-1 border-b" style={{ color: C.accent + "80", borderColor: C.border }}>Firestore Collection</h2>
                  <Field label="Collection Name *" value={form.collection_name} onChange={v => setField("collection_name", v)} placeholder="suppliers" hint="Firestore collection for suppliers" />
                  <div className="flex items-center gap-2 px-3 py-2 border" style={{ borderColor: C.border, backgroundColor: C.bg }}>
                    ...
                    <span className="text-[10px] font-mono" style={{ color: C.dim }}>
                      ...
                      {" "}→ <span style={{ color: "#60a5fa" }}>{form.collection_name || "suppliers"}</span>
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 pt-2">
                  <button onClick={handleTest} disabled={testing || saving}
                    ...
                    {testing ? <Loader2 className="size-3 animate-spin" />
                      : testResult === "ok" ? <CheckCircle2 className="size-3" style={{ color: "#34d399" }} />
