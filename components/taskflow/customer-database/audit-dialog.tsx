...
function runAuditLogic(customers: Customer[]): AuditResult {
  ...
  const sameTsaGroupIds = buildGroups(sameTsaPairs, Array.from(sameTsaInvolvedIds));
  const crossTsaGroupIds = buildGroups(crossTsaPairs, Array.from(crossTsaInvolvedIds));

  const duplicateGroups: DuplicateGroup[] = [
    ...sameTsaGroupIds.map((ids, i) => {
      const members = ids.map((id) => byId.get(id)!).filter(Boolean);
      const reasons: string[] = [];
      if (members.length >= 2 && norm(members[0].contact_person) === norm(members[1].contact_person))
        reasons.push("contact");
      if (members.length >= 2 && norm(members[0].email_address) === norm(members[1].email_address))
        reasons.push("email");
      return {
        id: `same-tsa-${i}`,
    ...
    ...crossTsaGroupIds.map((ids, i) => ({
      ...
      matchReason: "Cross-TSA — company + contact + email match across different TSAs",
      customers: ids.map((id) => byId.get(id)!).filter(Boolean),
    })),
  ];

  const duplicateIds = new Set<number>([...sameTsaInvolvedIds, ...crossTsaInvolvedIds]);
  const missingType = customers.filter((c) => !norm(c.type_client) && norm(c.status));
  const missingStatus = customers.filter((c) => !norm(c.status) && norm(c.type_client));
    ...
    line.type === "success" ? "text-emerald-400"
      : line.type === "warn" ? "text-amber-400"
        : line.type === "error" ? "text-red-400"
          : line.type === "cmd" ? "text-cyan-300 font-semibold"
            : "text-zinc-300";
  const prefix =
    line.type === "cmd" ? "$ "
      : line.type === "success" ? "✓ "
...
export function DuplicateGroupCard({ group }: { group: DuplicateGroup }) {
  ...
  return (
    <div className={cn(
      ...
      <p className="text-[11px] text-muted-foreground italic">{group.matchReason}</p>
      <div className="space-y-2">
        {group.customers.map((c, i) => (
          <div key={c.id} className="rounded-md border px-3 py-2 text-[12px] space-y-0.5 bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-700">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="font-semibold uppercase text-foreground">{c.company_name}</span>
              {c.account_reference_number && (
                <span className="text-muted-foreground font-mono text-[11px]">{c.account_reference_number}</span>
              )}
            </div>
            <div className="flex flex-wrap gap-x-4 gap-y-0.5 text-muted-foreground">
              <span>{c.contact_person || "—"}</span>
              <span>{c.email_address || "—"}</span>
              <span className="text-[11px] font-mono">{c.referenceid || "—"}</span>
            </div>
          </div>
        ))}
      </div>
...
function IssueBadgeCard({
  ...
  return (
    <div className={cn("rounded-lg border px-4 py-3 flex items-center gap-3", colorClass)}>
      {icon}
      <div>
...
export function AuditDialog({
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      await addLog(`Loaded ${customers.length.toLocaleString()} customer records.`, "info", 420);
      await addLog("─────────────────────────────────────────────", "info", 600);
      await addLog("Checking same-TSA duplicates (company + contact/email)...", "cmd", 850);
      ...
      const sameTsaGroups = result.duplicateGroups.filter((g) => g.type === "same-tsa");
      const crossTsaGroups = result.duplicateGroups.filter((g) => g.type === "cross-tsa");
      ...
      await addLog("Checking cross-TSA duplicates (company + contact + email)...", "cmd", 700);
      ...
      if (result.missingType.length > 0) {
        await addLog(`${result.missingType.length} record(s) missing type_client.`, "warn", 400);
      } else {
      ...
      if (result.missingStatus.length > 0) {
        await addLog(`${result.missingStatus.length} record(s) missing status.`, "warn", 400);
      } else {
        await addLog("All records have status set.", "success", 400);
      }

      await addLog("─────────────────────────────────────────────", "info", 600);
      const totalIssues = result.duplicateIds.size + result.missingType.length + result.missingStatus.length;
      await addLog(
        totalIssues === 0
  ...
  const renderTerminal = () => (
    <div className="flex flex-col h-full gap-4">
      <div className="rounded-xl overflow-hidden border border-zinc-700 bg-zinc-950 flex flex-col flex-1 min-h-0">
        <div className="flex items-center gap-2 px-4 py-2.5 bg-zinc-900 border-b border-zinc-800">
          ...
          <div className="flex-1 text-center">
            <span className="font-mono text-[11px] text-zinc-400 tracking-wide">customer-audit — bash</span>
          </div>
      ...
      <div className="flex justify-end gap-2 shrink-0">
        ...
        <Button disabled={!isDone} onClick={() => setPhase("results")} className="gap-1.5">
  ...
  const renderResults = () => {
    ...
    const { duplicateGroups, missingType, missingStatus, duplicateIds } = auditResult;
    const hasAnyIssue = duplicateGroups.length > 0 || missingType.length > 0 || missingStatus.length > 0;

    return (
      <div className="flex flex-col h-full gap-4">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 shrink-0">
          <IssueBadgeCard label="Dup Groups" count={duplicateGroups.length} icon={<Users className="size-5 text-red-500" />} colorClass="border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-950/40" />
          <IssueBadgeCard label="Dup Records" count={duplicateIds.size} icon={<XCircle className="size-5 text-red-400" />} colorClass="border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-950/40" />
          <IssueBadgeCard label="Missing Type" count={missingType.length} icon={<AlertTriangle className="size-5 text-amber-500" />} colorClass="border-amber-200 bg-amber-50 dark:border-amber-800 dark:bg-amber-950/40" />
          <IssueBadgeCard label="Missing Status" count={missingStatus.length} icon={<AlertTriangle className="size-5 text-amber-500" />} colorClass="border-amber-200 bg-amber-50 dark:border-amber-800 dark:bg-amber-950/40" />
        </div>
              ...
              {(missingType.length > 0 || missingStatus.length > 0) && (
                <div className="space-y-2">
                  ...
                  {[
                    ...
                    list.length > 0 ? (
                      <div key={label} className="rounded-lg border border-amber-300 bg-amber-50 dark:border-amber-800 dark:bg-amber-950/40 p-4 space-y-2">
                        <div className="flex items-center gap-2">
                        ...
                        <div className="space-y-1.5">
                          {list.slice(0, 6).map((c) => (
                            <div key={c.id} className="text-[12px] bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded px-3 py-1.5">
                              <span className="font-semibold uppercase">{c.company_name}</span>{" "}
                              <span className="text-muted-foreground">— {c.contact_person || "—"}</span>
                            </div>
                          ))}
                          {list.length > 6 && (
                            <p className="text-[11px] text-muted-foreground pl-1">+ {list.length - 6} more</p>
                          )}
                        </div>
                      </div>
                    ) : null
        ...
        <div className="flex justify-between gap-2 shrink-0">
          <Button variant="ghost" size="sm" onClick={() => setPhase("terminal")}>← Back to Log</Button>
  ...
  const renderConfirm = () => {
    ...
    const sameTsa = auditResult.duplicateGroups.filter((g) => g.type === "same-tsa").length;
    const crossTsa = auditResult.duplicateGroups.filter((g) => g.type === "cross-tsa").length;
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-6 text-center">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground max-w-sm">
              ...
              <span className="font-medium text-foreground">Customer Audits</span>{" "}
            ...
            {dupeGroups > 0 && (
              <>
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold tabular-nums">{dupeGroups}</span>
                </div>
                {sameTsa > 0 && (
            ...
            {auditResult.missingType.length > 0 && (
              <>
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold tabular-nums">{auditResult.missingType.length}</span>
                </div>
              </>
            )}
            {auditResult.missingStatus.length > 0 && (
              <>
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold tabular-nums">{auditResult.missingStatus.length}</span>
                </div>
              </>
            )}
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-muted-foreground mt-0.5">
                {phase === "terminal" && "Scanning customer database for issues..."}
                {phase === "results" && "Issues found — review before proceeding"}
            ...
            <div className="ml-auto flex items-center gap-1.5">
              {phaseOrder.map((p, i) => (
                <span key={p} className={cn(
                  ...
                  phase === p ? "bg-primary"
                    : i < phaseOrder.indexOf(phase) ? "bg-primary/40"
